         ...
         <div className="flex flex-col items-start gap-4">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              <input type="text" placeholder="Search" className="border border-gray-300 rounded px-3 py-1 text-sm rounded-md h-10" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
             
            </div>
          <div className="flex items-center gap-2">
            <button className="flex items-center gap-2 p-2 rounded-lg hover:bg-[#F5F5F5] transition duration-500 h-10">
            <Link to="/canvas" className="flex items-center gap-2"> <Plus className="w-4 h-4"/>Create Workflow</Link>
            </button></div>
          </div>
        </div>
